/**
 * 本屋大賞受賞作・ノミネート作に登場する作家のおすすめ記事を一括生成する。
 *
 * 実行例:
 *   npx tsx scripts/generate-honya-taisho-author-articles.ts --dry-run
 *   npx tsx scripts/generate-honya-taisho-author-articles.ts --limit 5
 *   npx tsx scripts/generate-honya-taisho-author-articles.ts --all
 *   npx tsx scripts/generate-honya-taisho-author-articles.ts --author "宮島未奈"
 */
...
import { normalizeAuthorName } from '../src/lib/honya-taisho-prize';
import { generateArticle } from './generate-article';

interface HonyaEntry {
  session: number;
  author: string;
}
...
const PRIZE_PATH = join(process.cwd(), 'src/data/honya-taisho-prize.json');
const NOMINEE_PATH = join(process.cwd(), 'src/data/honya-taisho-nominee.json');
const LOG_PATH = join(process.cwd(), 'scripts/honya-taisho-author-article-progress.json');
...
function loadAllEntries(): HonyaEntry[] {
  const prize = JSON.parse(readFileSync(PRIZE_PATH, 'utf-8')) as HonyaData;
  const nominee = JSON.parse(readFileSync(NOMINEE_PATH, 'utf-8')) as HonyaData;
...
async function main(): Promise<void> {
  ...
  console.log(`本屋大賞関連作家: ${authors.length}人`);
