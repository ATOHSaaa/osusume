/**
 * 「直木賞のすべて」から直木賞候補作一覧を取得し JSON を生成する。
 * 実行: npx tsx scripts/fetch-naoki-nominee-data.ts
 */
import * as cheerio from 'cheerio';
import { writeFileSync } from 'node:fs';
import { resolve } from 'node:path';

export interface NaokiNomineeEntry {
  session: number;
  period: string;
  author: string;
  title: string;
...
const SOURCE_BASE = 'https://prizesworld.com/naoki/ichiran';
const OUTPUT_PATH = resolve('src/data/naoki-nominee.json');
...
function parsePage(html: string, sourceUrl: string): NaokiNomineeEntry[] {
  const $ = cheerio.load(html);
  const entries: NaokiNomineeEntry[] = [];
...
async function main(): Promise<void> {
  const allEntries: NaokiNomineeEntry[] = [];
  ...
  if (allEntries.length < 500) {
    throw new Error(`Expected 500+ entries, got ${allEntries.length}`);
  }
