/**
 * Wikipedia「直木賞」受賞者一覧テーブルを取得し JSON を生成する。
 * 実行: npx tsx scripts/fetch-naoki-prize-data.ts
 */
import * as cheerio from 'cheerio';
import { writeFileSync } from 'node:fs';
import { resolve } from 'node:path';

export interface NaokiPrizeEntry {
  session: number;
  period: string;
  author: string;
  title: string;
...
const WIKIPEDIA_URL = 'https://ja.wikipedia.org/wiki/%E7%9B%B4%E6%9C%A8%E8%B3%9E';
const OUTPUT_PATH = resolve('src/data/naoki-prize.json');
...
function parseTables(html: string): NaokiPrizeEntry[] {
  const $ = cheerio.load(html);
  const entries: NaokiPrizeEntry[] = [];
