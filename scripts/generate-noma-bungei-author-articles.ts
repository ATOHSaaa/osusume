/**
 * 野間文芸賞受賞作・候補作に登場する作家のおすすめ記事を一括生成する。
 *
 * 実行例:
 *   npx tsx scripts/generate-noma-bungei-author-articles.ts --dry-run
 *   npx tsx scripts/generate-noma-bungei-author-articles.ts --limit 5
 *   npx tsx scripts/generate-noma-bungei-author-articles.ts --all
 */
...
import { normalizeAuthorName } from '../src/lib/noma-bungei-prize';
...
const PRIZE_PATH = join(process.cwd(), 'src/data/noma-bungei-prize.json');
const NOMINEE_PATH = join(process.cwd(), 'src/data/noma-bungei-nominee.json');
const LOG_PATH = join(process.cwd(), 'scripts/noma-bungei-author-article-progress.json');
...
function loadAllEntries(): NomaEntry[] {
  const prize = JSON.parse(readFileSync(PRIZE_PATH, 'utf-8')) as NomaData;
  const nominee = JSON.parse(readFileSync(NOMINEE_PATH, 'utf-8')) as NomaData;
...
async function main(): Promise<void> {
  ...
  console.log(`野間文芸賞関連作家: ${authors.length}人`);
