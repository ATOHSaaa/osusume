/**
 * 谷崎潤一郎賞受賞作・候補作に登場する作家のおすすめ記事を一括生成する。
 *
 * 実行例:
 *   npx tsx scripts/generate-tanizaki-junichiro-author-articles.ts --dry-run
 *   npx tsx scripts/generate-tanizaki-junichiro-author-articles.ts --limit 5
 *   npx tsx scripts/generate-tanizaki-junichiro-author-articles.ts --all
 */
...
import { normalizeAuthorName } from '../src/lib/tanizaki-junichiro-prize';
import { generateArticle } from './generate-article';

interface TanizakiJunichiroEntry {
  session: number;
  author: string;
}

interface TanizakiJunichiroData {
  entries: TanizakiJunichiroEntry[];
}
...
const PRIZE_PATH = join(process.cwd(), 'src/data/tanizaki-junichiro-prize.json');
const NOMINEE_PATH = join(process.cwd(), 'src/data/tanizaki-junichiro-nominee.json');
const LOG_PATH = join(process.cwd(), 'scripts/tanizaki-junichiro-author-article-progress.json');
...
function loadAllEntries(): TanizakiJunichiroEntry[] {
  const prize = JSON.parse(readFileSync(PRIZE_PATH, 'utf-8')) as TanizakiJunichiroData;
  const nominee = JSON.parse(readFileSync(NOMINEE_PATH, 'utf-8')) as TanizakiJunichiroData;
...
async function main(): Promise<void> {
  ...
  console.log(`谷崎潤一郎賞関連作家: ${authors.length}人`);
